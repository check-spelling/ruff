...
Here, the comment is dangling because it is preceded by `[`, which is a non-trivial token but not a
node, and  followed by `]`, which is also a non-trivial token but not a node. In the `FormatExprList`
